...
for(alpha in 1:length(alphas))
{
  for(trial in 1:num_of_trials){
    ...
    for (index in 1:nrow(cal_probs)) {
      cal_probs[index,] <- cumsum(cal_probs[index,])
    ...
    for (index in 1:nrow(val_probs)) {
      val_probs[index,] <- cumsum(val_probs[index,])
...
plot(alphas,average_cvg_opi,type = "o",col = "red",xlab = "miscoverage",ylab = "marginal coverage",lty = 1)

points(alphas,average_cvg_ops, col="blue", pch="*")
lines(alphas,average_cvg_ops,col = "blue",lty = 2)

plot(alphas,avg_set_size_opi,ylim = c(2,3),type = "o",col = "red",xlab = "miscoverage",ylab = "set size",lty = 1)

points(alphas,avg_set_size_ops, col="blue", pch="*")
lines(alphas,avg_set_size_ops,col = "blue",lty = 2)
...
plot(alphas,opi_conditional_coverage,ylim = c(0.0,0.25),type = "o",col = "red",xlab = "miscoverage",ylab = "CCV",lty = 1)

points(alphas,ops_conditional_coverage, col="blue", pch="*")
lines(alphas,ops_conditional_coverage,col = "blue",lty = 2)
...
plot(alphas,opi_conditional_coverage,ylim = c(0.0,0.15),type = "o",col = "red",xlab = "miscoverage",ylab = "CCV",lty = 1)
...
sigma <- matrix(c(1,0.1,0.1,0.1,1,0.1,0.1,0.1,1),ncol = d)
for (index in 1:N) {
  
  if(labels.generated[index] == 1)
    df[index,] <- mvrnorm(1,c(-1,0,0),sigma)
  else if(labels.generated[index] == 2)
    df[index,] <- mvrnorm(1,c(-1,-1,1),sigma)
  else if(labels.generated[index] == 3)
    df[index,] <- mvrnorm(1,c(0,-1,1),sigma)
  else
    df[index,] <- mvrnorm(1,c(1,-1,1),sigma)
}
...
plot(alphas,avg_set_size_opi,type = "o",col = "red",xlab = "miscoverage",ylab = "set size",lty = 1)
